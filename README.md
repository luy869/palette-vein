...
壁紙検索サイト（Wallhavenなど）は壁紙を探すことには向いています。しかし、その中から自分好みの作品を見つけるレコメンド機能は**ブラックボックス化**しており、Spotifyや各種SNSと同じ問題を抱えています。さらに、ほとんどのサービスは海外運営で、日本人ユーザーが親しみやすい設計ではありません。

より根本的な課題は、**「なぜそれがおすすめされているのか」が見えないために、自分の嗜好を理解したり、意図的に広げたりすることができない**という点です。推薦理由が透明なら、「自分は実はこういう色彩が好きなんだ」「この構図に惹かれている」といった気づきが生まれ、新しい好みの領域へも自然に進めるようになります。

Palette_Veinは、**推薦理由を可視化する**ことで、この課題を解決します。いいね・スキップを重ねるほど「自分が何を好むのか」が段階的に見え、その理由も同時に理解できるようになります。詳細は [PaletteVein_design.md](./PaletteVein_design.md) を参照してください。

---

## 技術選定とその意図

各技術を、なぜそれを選んだかという背景とともに説明します。詳細は [PaletteVein_design.md](./PaletteVein_design.md#技術選定) を参照してください。

- **Go（バックエンド）**: 好みデータの蓄積に伴い検索処理が増加します。実行速度と並列処理性能に優れたGoを採用し、スケーラビリティと高速性を確保しました。

- **Python + CLIP（AI処理）**: 壁紙のタグは作者が指定したメインモチーフに偏り、雰囲気・構図・細部の描写といった情報が欠落しがちです。CLIPは画像から雑多な特徴を統合的に捉えられるため、好みの細やかな表現に適しています。

- **gRPC（Go-Python連携）**: Python側を独立したCLIPサービスとして実装し、GoからgRPCで呼び出します。Protobufによる型安全性と高速なバイナリ通信により、連携の堅牢性を実現しました。将来的にはGPUサーバーへ分離可能です。

- **PostgreSQL + pgvector（データベース）**: メタデータ用RDBとベクトルストアを統合管理できるため、運用負荷が低く、ナレッジも豊富です。pgvectorによりCLIP出力ベクトルを同一DB上で直接扱えます。

- **React（フロントエンド）**: モダンで成熟したフレームワークです。既存の使用経験があるため、UIの透明性（推薦理由の可視化）に集中できました。
