...
**GPU互換（実測確定）**:
- torch 2.7 **cu128** = sm_75以上（75/80/86/90/100/120）→ **1650S=対応、1070(Pascal sm_61)=非対応**。
  cu128 のまま行くなら CLIP を **1650S に固定**（`_select_device` は空きVRAM最大を選ぶので放置すると8GBの1070を選んで `no kernel image` でクラッシュ）。
- torch 2.7 **cu126** = sm_50,60,70,75,80,86,90 → sm_60含むので **1070 も動く見込み**（6.0 cubin が 6.1 デバイスで動くマイナー互換・実機要確認）。
  本番だけ cu126 にすれば 1070/1650S 両方使える（5080は非対応なので dev=cu128/prod=cu126 と分かれる）。
- EVA02-B/16 は約1.5GBで1650S(4GB)に収まる。最簡=cu128+1650S。詳細は DEPLOY.md「重要・GPU互換」。
